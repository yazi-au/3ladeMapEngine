...
//        g2d.fillRect(0, 0, frameBuffer.getWidth(), frameBuffer.getHeight());
    ...
    public void pushImage(Player player, Monitor monitor,ArrayList<MapIcon> icons){
        if(monitor instanceof HoldMonitor){
            HoldMonitor hold = (HoldMonitor) monitor;
            SendMap.sendMap(player, hold.mapId, frameBuffer, icons);
            ...
            for (int i = 0; i < plane.mapIDs.size(); i++) {
                ...
                SendMap.sendMap(player,plane.mapIDs.get(i),frameBuffer.getSubimage(x1, y1, 128, 128), icons);
            }
        }
    }
}
